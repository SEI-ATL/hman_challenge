...
secret_words = ["destiny", "osiris", "zavala", "vanguard", "fallen", "taken", "cabal", "vex", "nine"]
# visual variables
hangman1 = '''
______
|    |
...
def continue_game():
    ...
    if hangman_guesses_left > 0:
        ...
        if len(guess) > 1 and errors < 1:
            print('It seems you have entered more than one character or guessed the wrong word.')
            errors += 1
            continue_game()
            return
        try:
            int(guess)
            print('It looks like you entered a number.')
            errors +=1
            continue_game()
            return
        except ValueError:
