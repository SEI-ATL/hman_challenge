...
def continue_game():
    ...
    if hangman_guesses_left > 0:
        ...
        if len(guess) > 1 and errors < 1:
            print('It looks like you accidentily entered more than one character or you guessed the wrong word.')
            errors += 1
            continue_game()
            return
        try:
            int(guess)
            print('It looks like you accidentily entered a number.')
            errors += 1
            continue_game()
            return
        except ValueError:
        ...
        lose()
# game loop that is going to continue running the game, call game_init() and initially run the continue_game(). Prompt user to see if they wanna continue playing

looping = True
game_init()
continue_game()
while looping:
    should_continue = input('Would you like to continue playing? yes or no\n').lower()
